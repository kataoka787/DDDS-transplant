...
class Edit extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post('d_id')) {
            ...
            $donor = $this->Donorbasetbl->getDonorBaseTblByDid($this->input->post('d_id'));
            if (!$donor) {
        ...
        $this->form_validation->run('donor/edit');
    ...
    public function conf()
    {
        ...
        if ($this->form_validation->run('donor/edit') == true) {
            /* XSS clean */
            ...
            // 2020/10/23 Start convert 2x->3x
            // $data = $this->session->set_flashdata('data', $this->data);
            $this->session->set_flashdata('edit_data', $this->data);
            // 2020/10/23 End convert 2x->3x
            ...
            $this->session->set_flashdata('input_data', $this->input->post(null, true));
            ...
            if (!$this->session->flashdata('id')) {
                ...
                $d_id = config_item('did_prefix') . date('Ymd') . "-" . ($this->session->flashdata('affiliation') - 1) . sprintf("%02d", ($donorCnt + 1));                
            } else {
            ...
            if ($this->session->flashdata('id')) {
                $this->data['page_title'] = config_item('page_donor_edit_conf');
            } else {
                $this->data['page_title'] = config_item('page_donor_regist_conf');
            }

            $this->load->vars($this->data);
            $this->load->view('header');
            $this->load->view('donor/conf');
            $this->load->view('footer');
        } else {

            if ($this->session->flashdata('id')) {
                $this->data['page_title'] = config_item('page_donor_edit');
            } else {
                $this->data['page_title'] = config_item('page_donor_regist');
            }
    ...
    {
        $this->load->view('header');
        $this->load->view('donor/error');
        $this->load->view('footer');
    }

    public function end()
    {
        $this->session->flashdata("edit_data") || redirect("donor/searchlist");
        if ($this->session->flashdata('id')) {
            $this->data['page_title'] = $this->config->item('page_donor_edit_comp');
        } else {
            $this->data['page_title'] = $this->config->item('page_donor_regist_comp');
        }
        $this->data['id'] = $this->session->flashdata('d_id');
