...
class TransplantUser extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->run('admin/transplantUser/search');
        $this->makeList();
        $this->data['prefList'] = $this->Prefmst->getPrefMst();
        $this->data['statusList'] = $this->Statusmst->getStatusMst();
        $this->load->vars($this->data);

        $this->load->view('header');
        $this->load->view('admin/transplantUser/searchlist');
    ...
    public function confirm()
    {
        ...
        if ($id !== null) {
            ...
            $data || redirect('admin/transplantUser');
            ...
            $this->load->vars($this->data);
            $this->load->view('header');
            $this->load->view('admin/transplantUser/confirm');
            $this->load->view('footer');
        } else {
            redirect('admin/transplantUser');
        }
    }
    ...
    public function edit()
    {
        ...
        /* Run form validation */
        $this->form_validation->run("admin/transplantUser/edit");
        ...
        $this->load->vars($this->data);
        $this->load->view('header');
        $this->load->view('admin/transplantUser/edit');
        $this->load->view('footer');
    }
    ...
    public function conf()
    {
        ...
        if ($this->form_validation->run("admin/transplantUser/edit") === true) {
            ...
            $this->data['pref'] = $this->Prefmst->getPrefMstById($inputData["pref_id"])->pref_name;
            $this->data['kana'] = $inputData["sei_kana"] . " " . $inputData["mei_kana"];
            $this->data['mail'] = $this->input->post('mail');
            $this->data['institution'] = $this->Institutionmst->getTransplantInstitutionMstById($inputData["institution"])->institution_name;
            ...
            $this->load->vars($this->data);
            $this->load->view('header');
            $this->load->view('admin/transplantUser/conf');
            $this->load->view('footer');
        } else {
            $this->data["prefList"] = $this->Prefmst->getPrefMst();
            $this->data["organs"] = $this->Institutionmst->getInstitutionOrgans($this->input->post("institution"));
            $this->data["works"] = $this->Workmst->getWorkMst();
            if ($this->input->post('pref_id')) {
                $this->data['institution'] = $this->Institutionmst->getTransplantInstitutionMstByPrefId($this->input->post('pref_id'));
            }
    ...
    public function update()
    {
        ...
        redirect('admin/transplantUser/edit');
    }
    ...
    private function createTransplantUser()
    {
        ...
        if (in_array(WORK_DDDS, $data["works"])) {
            ...
            $folderName = $displayData["institution"] . "_" . $organFolder . "_" . $data["sei"] . $data["mei"];
        ...
        redirect('admin/transplantUser/end');
    }
    ...
    private function updateTransplantUser()
    {
        ...
        $after = array(
            "pref_name" => $this->Prefmst->getPrefMstById($input_data['pref_id'])->pref_name,
            "institution_mst_id" => $input_data['institution'],
            "institution_name" => $this->Institutionmst->getTransplantInstitutionMstById($input_data['institution'])->institution_name,
            "sei" => $input_data['sei'],
            "mei" => $input_data['mei'],
    ...
    private function updateTpUserFolder($before, &$after)
    {
        ...
        if ($before['works'] != $after['works']) {
            ...
            if (empty($before["boxfolder_id"]) && in_array(WORK_DDDS, $after["works"])) {
                ...
                /* Create transplant user box folder */
                $folderName = $after['institution_name'] . "_" . $organs . "_" . $after["sei"] . $after["mei"];
    ...
    private function updateAccountInfo($before, &$after)
    {
        ...
        /* Update doctorTbl (institution) */
        if ($before["institution_name"] != $after["institution_name"]) {
            $update = array(
                "institution_mst_id" => $after['institution_mst_id'],
                "updated_at" => date('Y-m-d H:i:s'),
            );
    ...
    private function updateAccountHistory($before, $after)
    {
        $change = "";
        /* Pref change */
        if ($before['pref_name'] != $after['pref_name']) {
            $change .= "[都道府県]\n" . $before['pref_name'] . " => " . $after['pref_name'] . "\n";
        }
        /* Organs change */
        if ($before['organs'] != $after['organs']) {
        ...
        /* Institution change */
        if ($before['institution_mst_id'] != $after['institution_mst_id']) {
            $change .= "[移植施設]\n" . $before['institution_name'] . " => " . $after['institution_name'] . "\n";
        }
        /* Name change */
        ...
        /* Password change */
        if (!empty($after['password']) && $before['password'] != $after['password']) {
            $change .= "[パスワード変更]\n";
        }
    ...
    public function end()
    {
        if ($this->session->userdata('disp_data')) {
            ...
            $this->load->vars($this->data);
            $this->load->view('header');
            $this->load->view('admin/transplantUser/end');
            $this->load->view('footer');
        } else {
            redirect('admin/transplantUser');
        }
    ...
    public function delete()
    {
        ...
        if ($accId !== null) {
            ...
            if ($doctor) {
                ...
                $this->Accountchangehistorytbl->insertAccountChangeHistoryTblData($insert);
                redirect('admin/transplantUser/delete_end');
            }
        }
        redirect('admin/transplantUser');
    }
    ...
    {
        $this->load->view('header');
        $this->load->view('admin/transplantUser/delete_end');
        $this->load->view('footer');
    }

    public function csv()
    {
        $doctorList = $this->Doctortbl->getTransplantDoctorTblSearch("", "", array(), "", "", "", $this->session->userdata('affiliation_mst_id'));
        $organs_mst = $this->Internalorgansmst->getInternalOrgansMst();
        $organs = array();
        foreach ($organs_mst as $key => $val) {
            $organs[$val->id] = $val->organ_name;
        }

        $count = count($organs);
        $csv = "名前,施設コード,移植施設名,施設区分,ステータス,メールアドレス,";
        ...
        foreach ($doctorList as $doctor) {
            $csv .= $doctor->sei . " " . $doctor->mei . $delimiter;
            $csv .= $doctor->SISETU_CD . $delimiter;
            $csv .= $doctor->institution_name . $delimiter;
            $csv .= INSTITUTION_KUBUN[$doctor->institution_kubun] . $delimiter;
            $csv .= $doctor->status . $delimiter;
            $csv .= $doctor->mail . $delimiter;
        ...
        $fname = config_item('download_transplant_prefix') . date("Ymd") . '.csv';
    ...
    public function makeList()
    {
        ...
        $offset = is_numeric($this->uri->segment(3, 0)) ? $this->uri->segment(3, 0) : 0;

        $config['base_url'] = base_url() . "admin/transplantUser";
        $config['total_rows'] = $this->Doctortbl->getTransplantDoctorTblSearchCount($prefId, $institution, $organs, $seiKana, $meiKana, $status, $this->session->userdata("affiliation_mst_id"));
        ...
        $this->data['list'] = $this->Doctortbl->getTransplantDoctorTblSearch($prefId, $institution, $organs, $seiKana, $meiKana, $status, $this->session->userdata("affiliation_mst_id"), $offset, config_item('transplant_user_search_list_count'));
    }
}
