...
class Tp_MyClasses
{
    ...
    public function loginCheck()
    {
        ...
        $ci_config = $CI->config->config;
        $no_check = $ci_config['tp_no_login_check'];
        $check_flg = true;
        ...
        if ($check_flg) {
            if (!$CI->session->userdata('is_login') || $CI->session->userdata('account_type_mst_id') !== '3') {
                redirect(base_url());
            }
        }
        ...
        if (in_array($class, $ci_config['check_admin'])) {
            if ($CI->session->userdata("admin_flg") != IS_ADMIN) {
                redirect('/search');
            }
        }
    }
